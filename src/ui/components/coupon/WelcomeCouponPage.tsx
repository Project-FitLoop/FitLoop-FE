...
export default function WelcomeCouponPage({ mode = 'page' }: WelcomeCouponPageProps) {
  ...
  return (
    <div
      className={isModal ? '' : 'min-h-screen'}
      style={{
    ...
    >
      ...
      <main className="px-4 pb-8 space-y-4">
        <section
          ...
          style={{ background: 'var(--bg-white)' }}
        >
          <p className="text-xs font-semibold text-gray-500 mb-1">포함된 쿠폰</p>
        ...
        <section
          ...
          style={{ background: 'var(--bg-white)' }}
        >
          <p className="text-xs font-semibold text-gray-500">어떻게 사용하나요?</p>
          ...
          <div className="mt-3 rounded-lg px-3 py-2 bg-gray-50 text-[11px] text-gray-600">
            ...
            <p className="mt-0.5">A. 웰컴 쿠폰팩은 한 계정당 1회만 발급 가능합니다.</p>
          </div>
        </section>

        {isModal && (
          <div
            className="pt-2 pb-4 sticky bottom-0 left-0 right-0"
            style={{
              background:
                'linear-gradient(180deg, rgba(255,255,255,0.4) 0%, rgba(255,255,255,1) 40%)',
      ...
      {/* 페이지 전체일 때만 고정 CTA 버튼 */}
      {!isModal && (
        <div
          className="fixed left-0 right-0 bottom-0 px-4 pb-4 pt-2"
          style={{
            background:
              'linear-gradient(180deg, transparent 0%, #f5f5f5 35%, #f5f5f5 100%)',
          }}
        >
          <button
            className="w-full py-3 rounded-full text-sm font-semibold shadow-md active:opacity-90"
            style={{ background: 'var(--primary)', color: 'var(--bg-white)' }}
            onClick={handleClaimAll}
          >
            웰컴 쿠폰팩 한 번에 받기
          </button>
